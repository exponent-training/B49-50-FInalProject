...
public class RoleServiceImpl implements RoleService {
	...
	@Override
	public ResponseEntity<?> addRoleInService(Roles role) {
		if (role == null) {
	...
	public ResponseEntity<?> updateRoleInService(Roles role) {
		...
		if (roleRepository.existsById(role.getId()) && roleRepository.findByRoleName(role.getRoleName())==null){
			logger.info("Updated role with id {} to {}", role.getId(), role.getRoleName());
			roleRepository.save(role);
			return new ResponseEntity<>("Updated role with id " + role.getId(), HttpStatus.OK);
		}else {
